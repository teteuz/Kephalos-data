<template>
 ...
 <!-- Section Content Generated (，items) -->
...
const parseInterview = (text) => {
 ...
 // Profile
      const bioMatch = block.match(/_Profile:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
...
const InterviewDisplay = {
 ...
 // 
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply from this platform)' || t === '[No response]'
    }
...
const activeStep = computed(() => {
 ...
 // items
  return steps[0] || { noLabel: '--', title: 'Pending start', status: 'todo', meta: '' }
})
