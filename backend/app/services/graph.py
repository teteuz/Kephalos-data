...
def generate_ontology():
    ...
    try:
        ...
        # Obter arquivos enviados (opcional)
        ...
        if has_files:
            ...
            if not document_texts:
                ...
                return jsonify({
                    "success": False,
                    "error": "Nenhum documento foi processado com sucesso. Verifique o formato do arquivo."
                }), 400
            
            project.total_text_length = len(all_text)
            ProjectManager.save_extracted_text(project.project_id, all_text)
            logger.info(f"Extração de texto concluída: {len(all_text)} caracteres")
        else:
            logger.info("Nenhum arquivo enviado — gerando ontologia apenas com a diretiva de simulação")
            document_texts = []
