...
Step2: ZEP entity reading/filtering and OASIS simulation setup/run (automated)
"""
...
INTERVIEW_PROMPT_PREFIX = "Based on your persona and all memories/actions, reply directly in text without using tools:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview prompt with prefix to avoid tool usage by agents.
...
def get_graph_entities(graph_id: str):
    """
    Retrieve all entities in a graph (filtered).

    Only nodes matching predefined entity types are returned (not all nodes with Label 'Entity').

    Query params:
        entity_types: comma-separated list of entity types (optional, additional filtering)
        enrich: whether to include edge information (default true)
    """
    try:
...
def create_simulation():
    ...
    Note: max_rounds and similar parameters are generated by LLM and do not need manual setting.
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "kephalosdata_xxxx",    // optional, defaults from project when not provided
        ...
        def run_prepare():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message="Starting environment preparation..."
                )
...
def get_simulation_config(simulation_id: str):
    ...
    返回包含：
        ...
        - generation_reasoning: LLM configuration reasoning summary
    """
    try:
        manager = SimulationManager()
